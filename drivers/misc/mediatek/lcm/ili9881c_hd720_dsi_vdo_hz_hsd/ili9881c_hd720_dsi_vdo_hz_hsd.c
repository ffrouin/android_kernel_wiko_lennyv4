...
#define LCM_DBG(fmt, arg...) \
    LCM_PRINT ("[ili9881c_hsd] %s (line:%d) :" fmt "\r\n", __func__, __LINE__, ## arg)
// ---------------------------------------------------------------------------
//  Local Variables
// ---------------------------------------------------------------------------
...
#define HSD_LCM_MIN_VOL 1000
#define HSD_LCM_MAX_VOL 1800
...
static void lcm_get_params(LCM_PARAMS *params)
{
    ...
    params->dsi.horizontal_sync_active                = 20;//16;///////////////20 20 4  20  14  6
    params->dsi.horizontal_backporch                = 40;//15;
    params->dsi.horizontal_frontporch                = 40;//6;
		params->dsi.horizontal_active_pixel = FRAME_WIDTH;

		// Bit rate calculation
		//params->dsi.PLL_CLOCK=205;
	params->dsi.PLL_CLOCK=250;//215;  //LINE </EGAFM-297> <change the mipi clock to reduce disturbing the wifi> <20160413> panzaoyan
...
static unsigned int lcm_compare_id(void)
{
	...
	rc = IMM_GetOneChannelValue(AUXADC_LCD_ID_CHANNEL, data, &tmp);
		...
		if((HSD_LCM_MIN_VOL <= iVoltage) && (iVoltage <HSD_LCM_MAX_VOL))
				return 1;
			else
				return 0;
		}
...
LCM_DRIVER ili9881c_hd720_dsi_vdo_hz_hsd_lcm_drv = 
{
	.name		= "ili9881c_hd720_dsi_vdo_hz_hsd",
