...
#define COE_LCM_MIN_VOL 0
#define COE_LCM_MAX_VOL 900
...
static void lcm_get_params(LCM_PARAMS *params)
{
    ...
    params->dsi.horizontal_frontporch                = 40;//6;
		...
		params->dsi.PLL_CLOCK=250;//280;  //LINE </EGAFM-297> <change the mipi clock to reduce disturbing the wifi> <20160413> panzaoyan
...
static unsigned int lcm_compare_id(void)
{
	...
	rc = IMM_GetOneChannelValue(AUXADC_LCD_ID_CHANNEL, data, &tmp);
		...
		if((COE_LCM_MIN_VOL <= iVoltage) && (iVoltage < COE_LCM_MAX_VOL))
				return 1;
			else
				return 0;
		}
...
LCM_DRIVER ili9881c_hd720_dsi_vdo_coe_ivo_lcm_drv = 
{
	.name		= "ili9881c_hd720_dsi_vdo_coe_ivo",
