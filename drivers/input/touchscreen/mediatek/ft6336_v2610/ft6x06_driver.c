...
static  int update_firmware_thread(void *priv)
{
    ...
    work_lock=1;
        ...
        {
            if( panel_vendor_buff == FTS_CTP_VENDOR_DIJING)
                ft6x06_tp_upgrade(ftbin_dj, sizeof(ftbin_dj));
	    else if(panel_vendor_buff == FTS_CTP_VENDOR_HOLITECH)
                ft6x06_tp_upgrade(ftbin_hl, sizeof(ftbin_hl));
            else if(panel_vendor_buff == FTS_CTP_VENDOR_FUXIN)
                ft6x06_tp_upgrade(ftbin_fx, sizeof(ftbin_fx));
	    else
		CTP_DBG("update_firmware_thread : Invalid vendor ID");
        }
        else
        {
...
{
    sprintf(fw_vendor_name, "%s", fts_get_vendor_name(panel_vendor_buff));
}
