...
#include <linux/poll.h> //linzhiqin add for backtouch
#define FTS_SUPPORT_TRACK_ID
#if defined(FTS_AUTO_TP_UPGRADE)
#include "ftbin_DJing.h"
...
//linzhiqin add for backtouch
static int backtouch_irq_key = 0;
int backtouch_state = 0;
int isDownAction = 0;
static DECLARE_WAIT_QUEUE_HEAD(backtouch_poll_wq);

static const struct i2c_device_id ft6x06_tpd_id[] = {{DRIVER_NAME,0},{}};
static const struct of_device_id ft6x06_dt_match[] = {
	{.compatible = "mediatek,ft5346"},
	{},
};
...
static  void tpd_down(tinno_ts_data *ts, int x, int y, int pressure, int trackID) 
{
	CTP_DBG("x=%03d, y=%03d, pressure=%03d, ID=%03d", x, y, pressure, trackID);
        ...
        //linzhiqin add for backtouch start
        if(x == tpd_keys_dim_local[3][0] && y == tpd_keys_dim_local[3][1] ){
            if (isDownAction != 1) {
	        CTP_DBG("Backtouch tpd_down!!!!!");
                backtouch_state = 1;
                isDownAction = 1;
                backtouch_irq_key = 1;
                wake_up(&backtouch_poll_wq);
            }
        }
        //linzhiqin add for backtouch end
...
static int tpd_gesture_handle(struct touch_info *cinfo)
{
	...
	{ 	
		...
		input_sync(tpd->dev);
		//linzhiqin add for backtouch start
        CTP_DBG("Backtouch light screen !!!!");
        backtouch_state = 3;
        backtouch_irq_key = 1;
        wake_up(&backtouch_poll_wq);
        //linzhiqin add for backtouch start
...
//linzhiqin add for backtouch start
 unsigned int backtouch_poll(struct file *file, poll_table *wait)
{
	unsigned int mask = 0;
	
	poll_wait(file, &backtouch_poll_wq, wait);
	if(backtouch_irq_key != 0){
...
static  int update_firmware_thread(void *priv)
{
	...
	if (NORMAL_BOOT == get_boot_mode() ){
		...
		CTP_DBG("boot mode is not normal ,no upgrade tp firmware\n");
	}
...
//@xuchunsheng added end in 09/07/2015
        ...
        if( bEnTGesture == 1) {
          ...
          CTP_DBG("resume enable gesture_wakeup.\n");
...
static void tpd_suspend(struct early_suspend *h)
 {
	...
	const char dataEnDoubleclick = 0x30; // Jiangde, bit5=char&nubmer, bit4=DoubleClick, bit0=Backward
	...
	if ( g_pts ){
	    ...
	    if(bEnTGesture == 1) {
             ...
             CTP_DBG("suspend enable gesture_wakeup.\n");
		tpd_halt = 1;
		while (iRetry) 
		{
                        mutex_lock(&g_pts->mutex);
