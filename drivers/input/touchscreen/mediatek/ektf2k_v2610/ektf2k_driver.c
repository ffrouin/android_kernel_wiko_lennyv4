...
static int PROXIMITY_SUSPEND_BY_POWER_KEY = 0;

struct kobject *elan_debug_kobj;

static s32 elan_touchps_sysfs_init(void);
static void elan_touchps_sysfs_deinit(void);
...
static ssize_t elan_sysfs_touchps_store(struct device *dev,struct device_attribute *attr, const char *buf, size_t count)
{
    if(sscanf(buf, "%u", &PROXIMITY) != 1)
    {
	printk("[touchps]: Invalid values\n");
		return -EINVAL;
    }
	printk("[touchps]: PROXIMITY =%d\n",PROXIMITY);
...
static ssize_t elan_sysfs_touchps_show(struct device *dev,struct device_attribute *attr, char *buf)
{
//	HALL_DEBUG("[hall_dev] cur_hall_status=%d\n", cur_hall_status);
	printk("[touchps]: elan_sysfs_touchps_show =%d\n",PROXIMITY);

	return sprintf(buf, "%d\n", PROXIMITY_STATE);
}

static DEVICE_ATTR(touchps, S_IRUGO|S_IWUSR, elan_sysfs_touchps_show, elan_sysfs_touchps_store);
s32 elan_touchps_sysfs_init(void)
{
    s32 ret ;
    
    elan_debug_kobj = kobject_create_and_add("touchps", NULL) ;
//    SET_INFO_LINE_INFO("Starting initializing gtp_debug_sysfs");
    if (elan_debug_kobj == NULL)
    {
//        GTP_ERROR("%s: subsystem_register failed\n", __func__);
        return -ENOMEM;
    }
    ret = sysfs_create_file(elan_debug_kobj, &dev_attr_touchps.attr);
    if (ret)
    {
...
void elan_touchps_sysfs_deinit(void)
{
//    sysfs_remove_file(goodix_debug_kobj, &dev_attr_shorttest.attr);
    sysfs_remove_file(elan_debug_kobj, &dev_attr_touchps.attr);
    kobject_del(elan_debug_kobj);
}
...
static int tpd_suspend(struct i2c_client *client, pm_message_t message)
{
	...
	PROXIMITY_SUSPEND_BY_POWER_KEY = 1;
...
static int tpd_resume(struct i2c_client *client)
{
    ...
    {
        if (PROXIMITY_SLEEP == 1&&PROXIMITY_SUSPEND_BY_POWER_KEY == 0)
        {
            PROXIMITY_SLEEP = 0;
	...
	PROXIMITY_SUSPEND_BY_POWER_KEY = 0;
#endif //TP_PROXIMITY_SENSOR_NEW
