...
u8  g_u8_vendorID = 0;      // Jiangde++
u8  g_u8_cfg_version = 0;   // Jiangde++
...
s32 gt1x_init_panel(void)
{
    ...
    g_u8_cfg_version = gt1x_config[0];  // Jiangde++
...
s32 gt1x_read_version(struct gt1x_version_info * ver_info)
{
    ...
    g_u8_vendorID = sensor_id;          // Jiangde++
...
static s32 gt1x_wakeup_sleep(void)
{
	...
	while (retry++ < 2) {
#ifdef CONFIG_GTP_GESTURE_WAKEUP
		GTP_INFO("gt1x_wakeup_sleep, gesture_enabled=%d, bEnTGesture=%d \n", gesture_enabled, bEnTGesture);
		if (bEnTGesture) { // gesture_enabled
...
#ifdef CONFIG_GTP_GESTURE_WAKEUP
    GTP_DEBUG("HJDDbg, gesture_clear_wakeup_data, gesture_enabled=%d, bEnTGesture=%d \n", gesture_enabled, bEnTGesture);
...
int gt1x_resume(void)
{
    ...
    GTP_DEBUG("HJDDbg, gt1x_resume, gt1x_halt=%d, bEnTGesture=%d \n", gt1x_halt, bEnTGesture);
    if (!bEnTGesture && !gt1x_halt){ // !gesture_enabled
              return 0;
    	}
#else
    if (!gt1x_halt){
