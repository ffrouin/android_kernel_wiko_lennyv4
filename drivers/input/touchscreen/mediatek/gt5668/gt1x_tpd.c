...
extern u8  g_u8_vendorID;      // Jiangde++
extern u8  g_u8_cfg_version;   // Jiangde++

#ifdef CONFIG_GTP_GESTURE_WAKEUP // Jiangde++
...
static s32 tpd_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    GTP_ERROR("5688 in tpd_i2c_probe");
    ...
    strcpy(Tg_buf,"mcs");
    input_set_capability(tpd->dev, EV_KEY, KEYCODE_KEYTP); // Jiangde++
#endif
    ...
    GTP_ERROR("5688 out tpd_i2c_probe");
	return 0;
}
...
static int tpd_event_handler(void *unused)
{
	...
	s32 ret = 0;
	u8 point_data[11] = { 0 };
	struct sched_param param = {.sched_priority = 4 }; // Jiangde, Pending, RTPM_PRIO_TPD-->4
...
static int tpd_local_init(void)
{
	...
	memset(gpDMABuf_va, 0, IIC_DMA_MAX_TRANSFER_SIZE);
		...
		GTP_ERROR("1 before i2c_add_driver");
	if (i2c_add_driver(&tpd_i2c_driver) != 0) {
		GTP_ERROR("unable to add i2c driver.");
		return -1;
	}
		GTP_ERROR("2 afer i2c_add_driver");
...
#ifdef defined(CONFIG_TPD_HAVE_BUTTON)
	if (tpd_dts_data.use_tpd_button) {
...
void gt5668_tpd_get_fw_version( char * fw_vendor_numb )
{
    sprintf(fw_vendor_numb, "%x", g_u8_cfg_version);
}


void gt5668_tpd_get_fw_vendor_name(char * fw_vendor_name)
{
    if (CTP_VENDOR_ID_HLT == g_u8_vendorID)
    {
        sprintf(fw_vendor_name, "%s", CTP_VENDOR_NAME_HLT); // Jiangde, Pendings
    }
}


void gt5668_ftm_force_update(char * ftm_update) {
    // Jiangde, Pendings
    return;
}
...
static struct tpd_driver_t tpd_device_driver = {
    ...
    .tpd_get_fw_version = gt5668_tpd_get_fw_version,
    .tpd_get_fw_vendor_name = gt5668_tpd_get_fw_vendor_name,
	.tpd_ftm_force_update=gt5668_ftm_force_update,	
};
