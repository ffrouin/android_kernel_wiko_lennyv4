...
static char *fts_get_vendor_name(int vendor_id)
{
    switch(vendor_id)
    {
        ...
        case FTS_CTP_VENDOR_YEJI:
            return "YEJI";
            break;
        case FTS_CTP_VENDOR_YEJI_WITH_RSENSE:
            return "NYEJI";
            break;
...
extern int ft6x06_get_firmware_type(void);
#ifdef  FTS_AUTO_TP_UPGRADE
...
static  int update_firmware_thread(void *priv)
{
    ...
    work_lock=1;
		if (NORMAL_BOOT == get_boot_mode()){
                    if( panel_vendor_buff == FTS_CTP_VENDOR_HUARUICHUAN)
                    {
			   ...
			   ft6x06_tp_upgrade(ftbin_yeji, sizeof(ftbin_yeji));
                    }else if(panel_vendor_buff == FTS_CTP_VENDOR_YEJI_WITH_RSENSE){
                        ft6x06_tp_upgrade(ftbin_yeji_Rsense, sizeof(ftbin_yeji_Rsense));
                    }
		}
		else{
