...
int ft6x06_get_firmware_type(void)
{
	int ret;
	uint8_t firmware_type;
	ret = fts_read_reg(0xA7, &firmware_type);
	if (ret < 0){
		CTP_DBG("i2c error, ret=%d\n", ret);
		return -1;
	}
	CTP_DBG("firmware_type=0x%X\n", firmware_type);
...
int ft6x06_get_vendor_version(tinno_ts_data *ts, uint8_t *pfw_vendor, uint8_t *pfw_version)
{
	...
	*pfw_version = ft6x06_get_fw_version(ts);
	...
	ret = ft6x06_get_vendor_from_bootloader(ts, pfw_vendor, &pfw_bl_version);
		if (ret){
			CTP_DBG("ft6x06_get_vendor_from_bootloader error, ret=%d\n", ret);
			return -EFAULT;
		}
	
	CTP_DBG("two fw_vendor=0x%X, fw_bl_version=0x%X\n", *pfw_vendor, pfw_bl_version);
	return 0;
}
