...
#ifdef FACTORY_UPDATE_FIRMWARE
struct task_struct *factory_update_thread = NULL;
static int factory_update_percent=-1;// -1==unknow ,0~100 =percent; 0xff =fail;
int factory_update_bin_size=0;
uint8_t* factory_update_bin = NULL;
#endif

#define DRIVER_NAME "EKTF2339"
...
#ifdef FACTORY_UPDATE_FIRMWARE
static int update_fw_handler_factory(void *unused);
#endif
...
#endif
        factory_update_thread = kthread_run(update_fw_handler_factory, 0, TPD_DEVICE);
  return  ret;
		  break;
#else
...
#ifdef FACTORY_UPDATE_FIRMWARE
factory_update_percent=percent;
#endif
    if (page == (249))
        MTK_TP_DEBUG("\n");
}
...
#ifdef IAP_PORTION
/// wanglj, DATE20150804, NOTE, Bug FADALTHA-127 LINE 
#ifdef CONFIG_PROJECT_WIKO_UNIFY
extern int g_tpdrv_update_state;
#endif  /* CONFIG_PROJECT_WIKO_UNIFY */
static int update_fw_handler(void *unused)
{
    int New_FW_ID;
    int New_FW_VER;
    ...
    MTK_TP_DEBUG("[ELAN] geroge FW_ID=0x%x,   New_FW_ID=0x%x \n",  FW_ID, New_FW_ID);
    MTK_TP_DEBUG("[ELAN] geroge FW_VERSION=0x%x,   New_FW_VER=0x%x \n",  FW_VERSION  , New_FW_VER);
#endif

    /* for firmware auto-upgrade*/
    if (New_FW_ID   ==  FW_ID)
    {
        if ((New_FW_VER & 0xff) > (FW_VERSION & 0xff)||(ftm_ekt2k_force_update))
            Update_FW_One(private_ts->client, RECOVERY);
...
#ifdef FACTORY_UPDATE_FIRMWARE
#define ISP_FLASH_SIZE	0x9000 //32KB
char * ektf2k_load_fw_from_file(void)
{
	char * buffer=NULL;

		   if (factory_update_bin_size > ISP_FLASH_SIZE||factory_update_bin_size<=0)
		   	{
		   	    printk("[elan] ektf2k_load_fw_from_file error 1 \n");
		        return NULL;
		   	}
		    if (factory_update_bin == NULL)
		    	{
		    	printk("[elan] ektf2k_load_fw_from_file error 2 \n");
		        return NULL;
		    	}
		    buffer = kmalloc(factory_update_bin_size, GFP_KERNEL);
		    if (buffer == NULL)
		    	{
		    	printk("[elan] ektf2k_load_fw_from_file error 3\n");
		        return NULL;
		    	}

		    if (copy_from_user(buffer, factory_update_bin, factory_update_bin_size))
		    {
		    printk("[elan] ektf2k_load_fw_from_file error 4 \n");
		        return NULL;
		    }
			return buffer;
}
static int update_fw_handler_factory(void *unused)
{
   ...
   // mt_eint_mask(CUST_EINT_TOUCH_PANEL_NUM);
	 disable_irq(touch_irq);
    printk("[elan] start fw update\n");
factory_update_percent=-1;
file_fw_data=ektf2k_load_fw_from_file();
if(file_fw_data==NULL)
{
factory_update_percent=0xff;
goto exit;
}
    {
        printk("FW_ID is different!");
       if( Update_FW_One(private_ts->client, RECOVERY)!=0)
       	{
		factory_update_percent=0xff;
	   	}
	   else
	   factory_update_percent=100;
    }
        kfree(file_fw_data);
exit:
	factory_update_bin=NULL;
	factory_update_bin_size=0;
// Reset Touch Pannel
   tpd_gpio_output(GTP_RST_PORT, 0);
...
/// wanglj, DATE20150804, NOTE, Bug FADALTHA-127 LINE 
#if defined (IAP_PORTION) /*&& !defined (CONFIG_PROJECT_WIKO_UNIFY)*/
     get_vendor_info();
		...
		&&(NORMAL_BOOT == get_boot_mode()))
    {
        ...
        if(IS_ERR(update_thread))
        {
            ...
            MTK_TP_DEBUG("failed to create kernel update thread: \n");
        }
    }
...
printk("geroge start update ftm \n");
	 update_thread = kthread_run(update_fw_handler, 0, TPD_DEVICE);
        if(IS_ERR(update_thread))
        {
            retval = PTR_ERR(update_thread);
            printk("failed to create kernel update thread: \n");
        }
		
       printk("geroge end update ftm \n");
   }
}
