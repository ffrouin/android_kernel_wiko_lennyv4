...
//#ifdef CONFIG_PROJECT_V3760
#if defined(CONFIG_PROJECT_V3760)||(CONFIG_PROJECT_V3730)
//linzhiqin add for backtouch
#define  GF_IOC_BACKTOUCH	_IOW(TOUCH_IOC_MAGIC, 101, int)

extern unsigned int backtouch_poll(struct file *file, poll_table *wait);
extern int backtouch_state;
#endif
...
//#ifdef CONFIG_PROJECT_V3760
        ...
        case GF_IOC_BACKTOUCH:
	     data = (void __user *)arg;
	     if (copy_to_user(data, &backtouch_state, sizeof(backtouch_state)))
             {
                err = -EFAULT;
             }    
	     break;
