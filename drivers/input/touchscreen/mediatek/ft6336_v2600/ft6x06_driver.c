...
#include "ftbin_helitai.h"
#include "ftbin_hongzhan.h"
...
static u8 tpd_suspend_by_power_key = 0;
...
static ssize_t ft_sysfs_touchps_store(struct device *dev,struct device_attribute *attr, const char *buf, size_t count)
{

	if(sscanf(buf, "%u", &tpd_proximity_flag) != 1)
	{
		printk("[touchps]: Invalid values\n");
		return -EINVAL;
	}
...
static ssize_t ft_sysfs_touchps_show(struct device *dev,struct device_attribute *attr, char *buf)
{
//	HALL_DEBUG("[hall_dev] cur_hall_status=%d\n", cur_hall_status);
	return sprintf(buf, "%d\n", tpd_proximity_detect);
}

static DEVICE_ATTR(touchps, S_IRUGO|S_IWUSR, ft_sysfs_touchps_show, ft_sysfs_touchps_store);

s32 ft_touchps_sysfs_init(void)
{
    s32 ret ;
    
    ft_debug_kobj = kobject_create_and_add("touchps", NULL) ;
//    SET_INFO_LINE_INFO("Starting initializing gtp_debug_sysfs");
    if (ft_debug_kobj == NULL)
    {
//        GTP_ERROR("%s: subsystem_register failed\n", __func__);
        return -ENOMEM;
    }
    ret = sysfs_create_file(ft_debug_kobj, &dev_attr_touchps.attr);
    if (ret)
    {
...
void ft_touchps_sysfs_deinit(void)
{
//    sysfs_remove_file(goodix_debug_kobj, &dev_attr_shorttest.attr);
    sysfs_remove_file(ft_debug_kobj, &dev_attr_touchps.attr);
    kobject_del(ft_debug_kobj);
}
...
static char *fts_get_vendor_name(int vendor_id)
{
    switch(vendor_id)
    {
        ...
        case FTS_CTP_VENDOR_HONGZHAN_1:
            return "HONGZHAN";
            break;
        ...
        case FTS_CTP_VENDOR_DEFAULT:
            return "DEFAULT";
            break;
		case FTS_CTP_VENDOR_HELITAI:
		case FTS_CTP_VENDOR_HELITAI_1:
			return "HELITAI";
        default:
            return "UNKNOWN";
            break;
...
static  int update_firmware_thread(void *priv)
{
	...
	{ 
         ...
         ft6x06_tp_upgrade(ftbin_dijing, sizeof(ftbin_dijing));
			...
			else if((panel_vendor_buff == FTS_CTP_VENDOR_HELITAI)||(panel_vendor_buff == FTS_CTP_VENDOR_HELITAI_1))				
			ft6x06_tp_upgrade(ftbin_helitai, sizeof(ftbin_helitai));
			else if((panel_vendor_buff == FTS_CTP_VENDOR_HONGZHAN)||(panel_vendor_buff == FTS_CTP_VENDOR_HONGZHAN_1))
			ft6x06_tp_upgrade(ftbin_hongzhan, sizeof(ftbin_hongzhan));
        ...
        TPD_DEBUG("not upgrade firmware factory mode and recovery mode");
       }
    work_lock = 0;
    kthread_should_stop();
    return NULL;
...
static void tpd_resume(struct early_suspend *h)
{
    ...
    if (tpd_proximity_flag == 1)
    {
        if(tpd_proximity_flag_one == 1&&tpd_suspend_by_power_key == 0)
        {
